...
//!
//! Port of `_histogram_kernel_vp` and `_histogram_kernel` from `bcrs_fast_worker.py`.
...
///
/// Used by the coarse grid solver. Assumes xs/ys have exactly n_cols/n_rows
...
pub fn lrih(
    ...
    for c in 0..=n {
        ...
        while let Some(&(sc, sh)) = stack.last() {
            ...
            let mut y1 = ys[row_idx.min(n_ys.saturating_sub(1))];
