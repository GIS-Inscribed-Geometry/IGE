//! BCRS refinement orchestrator: Stage 3-7 pipeline orchestrator.
//!
//! OpenEvolve target: ``--target bcrs/refine.rs --mode balanced``
//!
//! Takes the top-k angle candidates from ``heuristic_candidates``, polishes
//! each with golden-section search (Stage 3), runs BCRS + SDF expansion
...
use crate::bcrs::certify::{certify_and_adjust, best_effort_shrink_to_cover, rect_sdf_max_poly};
use crate::bcrs::fallback::conservative_inner_fallback;
use crate::bcrs::polish::{bcrs_expand_at_angle, polish_angle};
