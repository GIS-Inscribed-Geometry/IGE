...
//!
//! OpenEvolve target: ``--target bcrs/certify.rs --mode balanced``
//!
//! These functions verify that a candidate inscribed rectangle is fully inside
//! the polygon (using the signed-distance field from ``axis_aligned::sdf``),
...
/// Certify that an oriented rect is fully inside ``poly``.
/// If the max SDF exceeds ``cert_eps``, shrink symmetrically from centre
/// up to ``cert_max_shrink`` fraction of the shorter half-side.
/// Returns ``(certified_rect, area)`` or ``None`` if unfixable.
...
/// Best-effort variant that allows a slightly looser shrink
/// (``max_sdf + cert_eps * 2.0``) and no max-shrink cap.
