...
fn collect_all_x_candidates(poly: &Polygon<f64>, g: &GroupedObs) -> Vec<f64> {
    ...
    if xs.len() <= MAX_CANDIDATES {
        return xs;
    }
    let step = (xs.len() - 1) as f64 / (MAX_CANDIDATES - 1) as f64;
    (0..MAX_CANDIDATES)
...
fn solve_core(poly: &Polygon<f64>, g: &GroupedObs, options: &LerOptions) -> Result<LerResult> {
    let (bx0, by0, bx1, by1) =
        poly_bbox(poly).ok_or_else(|| LirError::InvalidPolygon("degenerate".into()))?;
    ...
    let best = (0..xs.len())
        .into_par_iter()
        .filter_map(|i| {
            let mut local_best: Option<(f64, f64, f64, f64, f64)> = None;
            ...
            for j in (i + 1)..xs.len() {
                ...
                let Some((y0, y1)) = find_largest_y_gap(&mut intervals, by0, by1) else {
                    continue;
                };
                if y1 <= y0 + EPS {
                    continue;
                ...
                let w = x1 - x0;
                let h = y1 - y0;
                if !aspect_ok(w, h, options) {
                    continue;
                }

                let area = w * h;
                if area > local_area + EPS {
                    local_area = area;
                    local_best = Some((x0, y0, x1, y1, area));
                }
            }
            local_best
        })
...
pub fn solve_ler_axis_aligned_with_lines(
    ...
    for p in polygon_obstacles {
        inputs.push(ObstacleInput::Polygon(p.clone()));
    }
...
/// For points-only inputs without an explicit solver preference,
/// dispatches to the faster DC solver.
/// For mixed obstacles or explicit sweep, falls back to the general sweep solver.
pub fn solve_ler_axis_aligned_mixed(
    ...
    if obstacles
        .iter()
        .all(|o| matches!(o, ObstacleInput::Point(_)))
    {
        let pts: Vec<Coord<f64>> = obstacles
            .iter()
            ...
            .collect();
        if let Ok(r) = point_dc::solve_ler_points_dc(poly, &pts, options) {
            return Ok(r);
        }
    }
    solve_core(poly, &group_inputs(obstacles), options)
}

/// Sweep solver for mixed obstacles (bypasses DC shortcut).
/// Always uses the O(m² × k) sweep, never dispatches to DC.
...
mod tests {
    ...
    fn no_obstacles_fills_box() {
        let r = solve_ler_axis_aligned_exact(&rp(0., 0., 10., 10.), &[], &opts()).unwrap();
        assert!(r.area > 99.0);
    }
    #[test]
    fn thin_vertical_wall_centre() {
        let r = solve_ler_axis_aligned_exact(
            &rp(0., 0., 10., 10.),
            &[rp(4.95, 0., 5.05, 10.)],
            &opts(),
        )
    ...
    #[test]
    fn two_vertical_walls() {
        let r = solve_ler_axis_aligned_exact(
            &rp(0., 0., 10., 10.),
            &[rp(2.95, 0., 3.05, 10.), rp(6.95, 0., 7.05, 10.)],
            &opts(),
        )
        .unwrap();
        assert!(r.area > 35.0);
    ...
    #[test]
    fn small_centre_obstacle() {
        let r =
            solve_ler_axis_aligned_exact(&rp(0., 0., 10., 10.), &[rp(4.5, 4.5, 5.5, 5.5)], &opts())
                .unwrap();
        assert!(r.area > 40.0);
    }
    #[test]
    fn obstacle_covers_entire_box() {
        let r =
            solve_ler_axis_aligned_exact(&rp(0., 0., 10., 10.), &[rp(0., 0., 10., 10.)], &opts())
                .unwrap();
        assert!(r.area < 1.0);
    }
    #[test]
    fn four_corner_obstacles() {
        let r = solve_ler_axis_aligned_exact(
            &rp(0., 0., 10., 10.),
            &[
                rp(0., 0., 3., 3.),
    ...
    #[test]
    fn result_inside_polygon() {
        let r = solve_ler_axis_aligned_exact(&rp(0., 0., 10., 10.), &[rp(3., 3., 7., 7.)], &opts())
            .unwrap();
        if let Some(rect) = &r.rect {
            assert!(rp(0., 0., 10., 10.).contains(&Point::new(
                (rect.x_min + rect.x_max) * 0.5,
                (rect.y_min + rect.y_max) * 0.5
            )));
        }
    }
    #[test]
    fn degenerate_polygon() {
        ...
        let r = solve_ler_axis_aligned_exact(&flat, &[], &opts());
        assert!(r.is_ok() && r.unwrap().area == 0.0);
    }
    #[test]
    fn obstacle_touching_left_wall() {
        let r =
            solve_ler_axis_aligned_exact(&rp(0., 0., 10., 10.), &[rp(0., 0., 3., 10.)], &opts())
                .unwrap();
        assert!(r.area > 60.0);
    }
    #[test]
    fn aspect_max_ratio_square() {
        let mut o = opts();
        o.max_ratio = 1.0;
        let r = solve_ler_axis_aligned_exact(&rp(0., 0., 10., 4.), &[], &o).unwrap();
        if let Some(rect) = &r.rect {
            let (w, h) = (rect.x_max - rect.x_min, rect.y_max - rect.y_min);
            assert!(w.max(h) / w.min(h) <= 1.02);
            assert!(r.area > 15.0);
        }
    }
    #[test]
    fn aspect_min_ratio_two() {
        let mut o = opts();
        o.min_ratio = 2.0;
        let r = solve_ler_axis_aligned_exact(&rp(0., 0., 10., 10.), &[], &o).unwrap();
        if let Some(rect) = &r.rect {
            let (w, h) = (rect.x_max - rect.x_min, rect.y_max - rect.y_min);
            assert!(w.max(h) / w.min(h) >= 1.98);
        }
    }

    // ── point obstacle tests ──
    #[test]
    fn single_point_obstacle() {
    ...
    #[test]
    fn backward_compat_lines_thickness() {
        let poly = rp(0., 0., 10., 10.);
        let line = LineString::from(vec![coord! { x: 5., y: 0. }, coord! { x: 5., y: 10. }]);
