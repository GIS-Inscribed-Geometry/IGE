...
fn dc(
    ...
    for i in 0..n {
        ...
        if is_left {
            pref_l[i + 1] = pref_l[i + 1].max(px);
        } else {
            pref_r[i + 1] = pref_r[i + 1].min(px);
        }
    }
    ...
    for i in (0..n).rev() {
        ...
        let (px, _, is_left) = by_y[i];
        if is_left {
            suff_l[i] = suff_l[i].max(px);
        } else {
            suff_r[i] = suff_r[i].min(px);
        }
    }
...
mod tests {
    ...
    fn dc_no_points() {
        ...
        let r = solve_ler_points_dc(&poly, &[], &opts()).unwrap();
        assert!(r.area > 99.0);
    }
    #[test]
    fn dc_single_point() {
        ...
        let r = solve_ler_points_dc(&poly, &pts, &opts()).unwrap();
        assert!(r.area > 20.0 && r.area < 80.0);
    }
    #[test]
    fn dc_two_points_same_y() {
        let poly = rp(0., 0., 10., 10.);
        let pts = vec![coord! { x: 3., y: 5. }, coord! { x: 7., y: 5. }];
        let r = solve_ler_points_dc(&poly, &pts, &opts()).unwrap();
        assert!(r.area > 0.0);
    }
    #[test]
    ...
    fn dc_matches_exact_random_50() {
        ...
        assert!(
            r_new.area >= r_old.area - 10.0,
            "DC area {:.2} < old area {:.2} by more than 10",
            r_new.area,
            r_old.area
        );
    }
    ...
    fn dc_matches_exact_random_multi() {
        ...
        for seed in 0..20 {
            ...
            let n = (rng.gen::<f64>() * 25.0 + 5.0) as usize;
            let mut pts = Vec::new();
            for _ in 0..n {
                let x = rng.gen::<f64>() * 98.0 + 1.0;
                let y = rng.gen::<f64>() * 98.0 + 1.0;
                pts.push(Coord { x, y });
            }
            let obs: Vec<Polygon<f64>> = pts
                .iter()
