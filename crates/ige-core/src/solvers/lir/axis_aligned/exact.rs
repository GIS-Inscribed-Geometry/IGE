...
use crate::tuning::{
    AA_EPS, AA_EXACT_BINARY_ITERS, AA_EXACT_GRID_CAP, AA_EXACT_REFINE_ITERS, AA_EXACT_TOP_SEEDS,
};
...
fn refine_continuous(
    ...
    if !rect_fully_contained(poly, x0, y0, x1, y1)
        || !aspect_ok(x1 - x0, y1 - y0, options.max_ratio)
    {
        return None;
    }
    for _ in 0..AA_EXACT_REFINE_ITERS {
        ...
        if (x0 - p.0).abs() + (y0 - p.1).abs() + (x1 - p.2).abs() + (y1 - p.3).abs() < 1e-10 {
            break;
        }
    }
    if !rect_fully_contained(poly, x0, y0, x1, y1)
...
fn push_top_seed(seeds: &mut Vec<Rectangle>, rect: &Rectangle) {
    ...
    if seeds.len() < AA_EXACT_TOP_SEEDS {
        seeds.push(rect.clone());
        return;
    }
    let mut min_idx = 0;
    let mut min_area = seeds[0].area();
    for (i, r) in seeds.iter().enumerate().skip(1) {
        let a = r.area();
        if a < min_area {
            min_area = a;
            min_idx = i;
...
fn best_discrete_support_rect(
    ...
    let partials: Vec<(Option<Rectangle>, f64, Vec<Rectangle>)> = (0..xs.len() - 1)
        ...
        .map(|i| {
            let mut best = None;
            let mut best_a = 0.0;
            let mut seeds: Vec<Rectangle> = Vec::new();
            let x0 = xs[i];
            for j in (i + 1)..xs.len() {
                let x1 = xs[j];
                let w = x1 - x0;
                ...
                let max_h = if options.max_ratio > 0.0 {
                    w * options.max_ratio
                } else {
                    f64::INFINITY
                };
                if w * (y_max - y_min).min(max_h) <= best_a + EPS {
                    continue;
                }
                for y0_idx in 0..ys.len() - 1 {
                    let y0 = ys[y0_idx];
                    let mh = (y_max - y0).min(max_h);
                    if w * mh <= best_a + EPS || mh <= EPS {
                        continue;
                    }
                    let yhv = y0 + mh;
                    let mut hi_idx = ys.len() - 1;
                    while hi_idx > y0_idx + 1 && ys[hi_idx] > yhv + EPS {
                        hi_idx -= 1;
                    }
                    if hi_idx <= y0_idx {
                        continue;
                    ...
                    if let Some(idx) = found {
                        let y1 = ys[idx];
                        let hh = y1 - y0;
                        if !aspect_ok(w, hh, options.max_ratio) {
                            continue;
                        }
                        let r = Rectangle {
                            x_min: x0,
...
pub fn solve_axis_exact(poly: &Polygon<f64>, options: &AxisAlignedOptions) -> Option<Rectangle> {
    ...
    let mut best = best_disc;
    let mut best_a = best.area();
    for seed in seeds {
