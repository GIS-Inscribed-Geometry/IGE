...
pub use containment::{contract_rect_to_boundary, rect_fully_contained};
pub use exact::solve_axis_exact;
pub use grid::{
    solve_axis_rect_bcrs as solve_axis_rect_fine, solve_axis_rect_bcrs_with_backend,
    solve_axis_rect_grid, solve_axis_rect_grid_with_backend, MaskBackend,
};
pub use histogram::{lrih, lrih_vp};
pub use sdf::{best_effort_shrink, certify_rect, polygon_sdf, rect_sdf_max};
pub use vertex_grid::{detect_polygon_type, solve_vertex_grid, AxisAlignedOptions};
    ...
    pub fn solve(
        ...
        match self {
            ...
            AxisAlignedSolver::UniformGrid => {
                let steps = if options.max_grid > 0 {
                    options.max_grid
                } else {
                    crate::tuning::AA_GRID_COARSE_STEPS
                };
                let result =
                    solve_axis_rect_grid(poly, steps, options.max_ratio, options.min_ratio)?;
